...
public class MealsRecordAction {
	...
	public @ResponseBody String export(HttpServletResponse response) {
		...
		try {
			// 获得文件名
			ModelTitle modelTitle = MealsRecordEntity.class.getAnnotation(ModelTitle.class);
			String filename = modelTitle.name();
//			System.out.println(filename);
			// 总行数
			long rowSize = PageHelper.count(() -> {
				mealsRecordService.selectList();
			});
...
//			map.put(5, area);// 所属行政区下拉框
			...
			// 导出
			input = ImportExcelUtil.excelModelbyClass(MealsRecordEntity.class, map, (int) rowSize, list);
			int index;
	...
	public @ResponseBody String mealsRecordListApp(HttpServletResponse response){
		response.setHeader("Access-Control-Allow-Origin", "*");
		List<MealsRecordEntity> list = mealsRecordService.selectList();
		String json = JSONHelper.toJSON(list);
		return json;
