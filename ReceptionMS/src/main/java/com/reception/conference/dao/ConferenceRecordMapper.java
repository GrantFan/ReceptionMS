...
public interface ConferenceRecordMapper {
	...
	@Select("<script>"
			...
			+ " and HOTEL = #{hotelName} "
			+ "</if>" 
			+ " <if test=\" '' != conference_name  and null != conference_name\"> "
			+ " and conference_name = #{conference_name} "
			+ "</if>" 
			+ " <if test=\" '' != begin_DATE  and null != begin_DATE\"> "
			+ " and USE_DATE &gt;= to_date(#{begin_DATE},'yyyy-mm-dd') "
			+ "</if>" 
			+ " <if test=\" '' != end_DATE  and null != end_DATE\"> "
			+ " and USE_DATE &lt;= to_date(#{end_DATE},'yyyy-mm-dd') "
			+ "</if>" 
